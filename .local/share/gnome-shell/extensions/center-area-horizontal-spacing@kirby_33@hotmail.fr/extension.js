/**
 * Fork of the Status Area Horizontal Spacing extension, but for the Center
 * area. Thank you to the original author: mathematical.coffee
 * 
 * CenterAreaHorizontalSpacing extension
 * v2.0
 *
 * This extension essentially modifies the "-natural-hpadding"
 * attribute of panel-buttons (i.e. indicators in the center area)
 * so that they can be closer together.
 *
 * The default is 12.
 *
 * It does this by using 'set_style' to override the '-natural-hpadding'
 * property of anything added to Main.panel._centerBox.
 * It listens to the 'add-actor' signal of Main.panel._centerBox to override
 * the style.
 *
 */
...
function overrideStyle(actor, secondTime) {
    // it could be that the first child has the center style class name.
    ...
    // a flag?
    if (!actor._centerAreaHorizontalSpacingSignalID) {
        actor._centerAreaHorizontalSpacingSignalID =
            actor.connect('style-changed', function () {
                ...
                if (currStyle && !currStyle.match(styleLine)) {
                    ...
                    actor.disconnect(actor._centerAreaHorizontalSpacingSignalID);
                    delete actor._centerAreaHorizontalSpacingSignalID;
                    overrideStyle(actor);
                }
            });
    }
...
function applyStyles() {
    ...
    /* set style for everything in _centerBox */
    ...
    /* connect signal */
    actorAddedID = Main.panel._centerBox.connect('actor-added',
...
function removeStyles() {
    /* disconnect signal */
    if (actorAddedID) {
        Main.panel._centerBox.disconnect(actorAddedID);
    }
